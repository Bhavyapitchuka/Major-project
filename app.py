...
from database import patient_login,admin_login, doctor_login,patient_reg,doctor_reg,add_symptoms,view_patient,dp_add,pp_act
...
def pp():
      data1=pp_act() 
      return render_template("pp.html",m1="sucess", data=data1)
...
@app.route("/dpadd", methods = ['GET','POST'])
def dpadd():
   if request.method == 'POST':      
      status = dp_add(request.form['did'],request.form['doctor'],request.form['pid'],request.form['pname'],request.form['pres'])
      if status == 1:
       return render_template("doctorhome.html",m1="sucess")
      else:
       return render_template("dpd.html",m1="failed") 
...
def acc1():
      ...
      accp(a, e,  f)
      return render_template("dpd.html",m1="sucess",did=a,doctor=b,pid=e,pname=f)
